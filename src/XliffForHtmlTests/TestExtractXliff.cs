   ...
   <trans-unit id=""single para"">
    ...
    <note>ID: single para</note>
			...
			foreach (XmlNode tu in body.ChildNodes)
			{
				...
				{
				case 0:
					Assert.AreEqual("lots of text", tu.Attributes["id"].Value);
					Assert.AreEqual(5, source.ChildNodes.Count);
					int index1 = 0;
					...
					break;
				case 1:
					Assert.AreEqual("single para", tu.Attributes["id"].Value);
					Assert.AreEqual(1, source.ChildNodes.Count);
					Assert.AreEqual(XmlNodeType.Text, source.ChildNodes[0].NodeType);
					Assert.AreEqual("This paragraph is independent of the others.", source.ChildNodes[0].InnerText);
					break;
				}
