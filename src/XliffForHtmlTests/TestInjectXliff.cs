  ...
  <body>
   ...
   <trans-unit id=""single para"">
    ...
    <note>ID: single para</note>
			...
			var index = 0;
			foreach (var node in divs[0].ChildNodes)
			{
				switch (index)
				{
				case 0:
					Assert.AreEqual("p", node.Name);
					Assert.AreEqual(0, node.Attributes.Count);
					Assert.AreEqual("Это первый абзац.", node.InnerHtml);
					break;
				case 1:
					Assert.AreEqual("p", node.Name);
					Assert.AreEqual(0, node.Attributes.Count);
					Assert.AreEqual("Это второй абзац. Это даже длиннее первого абзаца.", node.InnerHtml);
					break;
				}
				++index;
			}
