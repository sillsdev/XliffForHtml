  ...
  <body>
   ...
   <trans-unit id=""Copyright.Notice"">
    ...
    <note>ID: Copyright.Notice</note>
			...
			var cells = translatedHtml.DocumentNode.SelectNodes("/html/body/table/tr/td");
			Assert.AreEqual(4, cells.Count);
			// No lang attribute added, original English text retained
  ...
  <body>
   <trans-unit id=""translate please"">
    ...
    <note>ID: translate this!</note>
		...
		{
			var injector = HtmlXliff.ParseMarkdown(@"## Bloom Notes & License {i18n=""license.title""}
...
You can disable this feature in the Settings area.</source>
    <target xml:lang=""es"">Bloom comprueba automáticamente si hay nuevas versiones y las descarga mientras trabaja.
Puede desactivar esta función en el área Configuración.</target>
    <note>ID: license.autoupdate.text</note>
			...
			// Markdown creates HTML fragments that are assumed to be inside <html><body>
			var elements = translatedHtml.DocumentNode.SelectNodes("/*");
			Assert.AreEqual(3, elements.Count);

			var h2 = elements[0];
			Assert.AreEqual("h2", h2.Name);
			Assert.AreEqual(3, h2.Attributes.Count);
			Assert.AreEqual("license.title", h2.Attributes["i18n"].Value);
			Assert.AreEqual("es", h2.Attributes["lang"].Value);
			Assert.AreEqual("es", h2.Attributes["xml:lang"].Value);
			Assert.AreEqual("Bloom Notas y Licencia", h2.InnerHtml);

			var h3 = elements[1];
			Assert.AreEqual("h3", h3.Name);
			Assert.AreEqual(3, h3.Attributes.Count);
			Assert.AreEqual("license.autoupdate", h3.Attributes["i18n"].Value);
			Assert.AreEqual("es", h3.Attributes["lang"].Value);
			Assert.AreEqual("es", h3.Attributes["xml:lang"].Value);
			Assert.AreEqual("Actualización automática", h3.InnerHtml);

			var para = elements[2];
			Assert.AreEqual("p", para.Name);
			Assert.AreEqual(3, para.Attributes.Count);
			Assert.AreEqual("license.autoupdate.text", para.Attributes["i18n"].Value);
			Assert.AreEqual("es", para.Attributes["lang"].Value);
			Assert.AreEqual("es", para.Attributes["xml:lang"].Value);
			Assert.AreEqual(@"Bloom comprueba automáticamente si hay nuevas versiones y las descarga mientras trabaja.
Puede desactivar esta función en el área Configuración.", para.InnerHtml);
		}

		[Test]
		public void TestMarkdownWithLinksAndLists()
		{
			var injector = HtmlXliff.ParseMarkdown(@"After you submit this report, we will contact you and help you work this out. In the meantime, here are some ideas: {i18n=""integrity.todo.ideas""}
 ...
 <file original=""test.html"" datatype=""html"" source-language=""en"" target-language=""fr"">
  <body>
   <trans-unit id=""integrity.todo.ideas"">
    ...
    <target xml:lang=""fr"">Une fois que vous aurez rempli ce signalement, nous vous contacterons et vous aiderons à résoudre le problème. D'ici là, vous pouvez essayer les actions suivantes :</target>
   ...
   <trans-unit id=""integrity.todo.ideas.AVG"">
    ...
    <target xml:lang=""fr"">AVG : <g id=""genid-2"" ctype=""x-html-a"" html:href=""https://support.avg.com/SupportArticleView?l=en_US&amp;urlname=How-to-exclude-file-folder-or-website-from-AVG-scanning"">Instructions de AVG</g>.</target>
    <note>ID: integrity.todo.ideas.AVG</note>
			...
			Assert.AreEqual("fr", para.Attributes["lang"].Value);
			Assert.AreEqual("fr", para.Attributes["xml:lang"].Value);
			Assert.AreEqual("integrity.todo.ideas", para.Attributes["i18n"].Value);
			Assert.AreEqual("Une fois que vous aurez rempli ce signalement, nous vous contacterons et vous aiderons à résoudre le problème. D'ici là, vous pouvez essayer les actions suivantes :", para.InnerText);

			var ul = elements[1];
			Assert.AreEqual("ul", ul.Name);
			Assert.AreEqual(0, ul.Attributes.Count);
			Assert.AreEqual(5, ul.ChildNodes.Count);

			Assert.AreEqual(HtmlNodeType.Text, ul.ChildNodes[0].NodeType);
			Assert.IsNotNullOrEmpty(ul.ChildNodes[0].InnerText);
			Assert.IsTrue(String.IsNullOrWhiteSpace(ul.ChildNodes[0].InnerText));

			Assert.AreEqual(HtmlNodeType.Text, ul.ChildNodes[2].NodeType);
			Assert.IsNotNullOrEmpty(ul.ChildNodes[2].InnerText);
			Assert.IsTrue(String.IsNullOrWhiteSpace(ul.ChildNodes[2].InnerText));

			Assert.AreEqual(HtmlNodeType.Text, ul.ChildNodes[4].NodeType);
			Assert.IsNotNullOrEmpty(ul.ChildNodes[4].InnerText);
			Assert.IsTrue(String.IsNullOrWhiteSpace(ul.ChildNodes[4].InnerText));

			var li0 = ul.ChildNodes[1];
			Assert.AreEqual("li", li0.Name);
			Assert.AreEqual(0, li0.Attributes.Count);
			Assert.AreEqual(1, li0.ChildNodes.Count);
			var span = li0.ChildNodes[0];
			Assert.AreEqual("span", span.Name);
			Assert.AreEqual(3, span.Attributes.Count);
			Assert.AreEqual("fr", span.Attributes["lang"].Value);
			Assert.AreEqual("fr", span.Attributes["xml:lang"].Value);
			Assert.AreEqual("integrity.todo.ideas.Reinstall", span.Attributes["i18n"].Value);
			Assert.AreEqual(1, span.ChildNodes.Count);
			Assert.AreEqual(HtmlNodeType.Text, span.ChildNodes[0].NodeType);
			Assert.AreEqual("Lancez à nouveau l'installateur Bloom et vérifiez si cette fois, il démarre correctement.", span.InnerHtml);

			var li1 = ul.ChildNodes[3];
			Assert.AreEqual("li", li1.Name);
			Assert.AreEqual(0, li1.Attributes.Count);
			Assert.AreEqual(4, li1.ChildNodes.Count);
			span = li1.ChildNodes[0];
			Assert.AreEqual(3, span.Attributes.Count);
			Assert.AreEqual("fr", span.Attributes["lang"].Value);
			Assert.AreEqual("fr", span.Attributes["xml:lang"].Value);
			Assert.AreEqual("integrity.todo.ideas.anti-virus", span.Attributes["i18n"].Value);
			Assert.AreEqual(1, span.ChildNodes.Count);
			Assert.AreEqual(HtmlNodeType.Text, span.ChildNodes[0].NodeType);
			Assert.AreEqual("Si cela ne règle pas le problème, il va vous falloir intervenir au niveau de votre programme antivirus.", span.InnerHtml);

			Assert.AreEqual(HtmlNodeType.Text, li1.ChildNodes[1].NodeType);
			Assert.IsNotNullOrEmpty(li1.ChildNodes[1].InnerText);
			Assert.IsTrue(String.IsNullOrWhiteSpace(li1.ChildNodes[1].InnerText));

			Assert.AreEqual(HtmlNodeType.Text, li1.ChildNodes[3].NodeType);
			Assert.IsNotNullOrEmpty(li1.ChildNodes[3].InnerText);
			Assert.IsTrue(String.IsNullOrWhiteSpace(li1.ChildNodes[3].InnerText));

			var ul2 = li1.ChildNodes[2];
			Assert.AreEqual("ul", ul2.Name);
			Assert.AreEqual(0, ul2.Attributes.Count);
			Assert.AreEqual(5, ul2.ChildNodes.Count);

			Assert.AreEqual(HtmlNodeType.Text, ul2.ChildNodes[0].NodeType);
			Assert.IsNotNullOrEmpty(ul2.ChildNodes[0].InnerText);
			Assert.IsTrue(String.IsNullOrWhiteSpace(ul2.ChildNodes[0].InnerText));

			Assert.AreEqual(HtmlNodeType.Text, ul2.ChildNodes[2].NodeType);
			Assert.IsNotNullOrEmpty(ul2.ChildNodes[2].InnerText);
			Assert.IsTrue(String.IsNullOrWhiteSpace(ul2.ChildNodes[2].InnerText));

			Assert.AreEqual(HtmlNodeType.Text, ul2.ChildNodes[4].NodeType);
			Assert.IsNotNullOrEmpty(ul2.ChildNodes[4].InnerText);
			Assert.IsTrue(String.IsNullOrWhiteSpace(ul2.ChildNodes[4].InnerText));

			var li2a = ul2.ChildNodes[1];
			Assert.AreEqual("li", li2a.Name);
			Assert.AreEqual(0, li2a.Attributes.Count);
			Assert.AreEqual(1, li2a.ChildNodes.Count);
			span = li2a.ChildNodes[0];
			Assert.AreEqual(3, span.Attributes.Count);
			Assert.AreEqual("fr", span.Attributes["lang"].Value);
			Assert.AreEqual("fr", span.Attributes["xml:lang"].Value);
			Assert.AreEqual("integrity.todo.ideas.AVAST", span.Attributes["i18n"].Value);
			Assert.AreEqual(3, span.ChildNodes.Count);
			Assert.AreEqual(HtmlNodeType.Text, span.ChildNodes[0].NodeType);
			Assert.AreEqual("AVAST : ", span.ChildNodes[0].InnerHtml);
			var anchor = span.ChildNodes[1];
			...
			var li2b = ul2.ChildNodes[3];
			Assert.AreEqual("li", li2b.Name);
			Assert.AreEqual(0, li2b.Attributes.Count);
			Assert.AreEqual(1, li2b.ChildNodes.Count);
			span = li2b.ChildNodes[0];
			Assert.AreEqual(3, span.Attributes.Count);
			Assert.AreEqual("fr", span.Attributes["lang"].Value);
			Assert.AreEqual("fr", span.Attributes["xml:lang"].Value);
			Assert.AreEqual("integrity.todo.ideas.AVG", span.Attributes["i18n"].Value);
			Assert.AreEqual(3, span.ChildNodes.Count);
			Assert.AreEqual(HtmlNodeType.Text, span.ChildNodes[0].NodeType);
			Assert.AreEqual("AVG : ", span.ChildNodes[0].InnerHtml);
			anchor = span.ChildNodes[1];
			Assert.AreEqual("a", anchor.Name);
			Assert.AreEqual(1, anchor.Attributes.Count);
			Assert.AreEqual("https://support.avg.com/SupportArticleView?l=en_US&amp;urlname=How-to-exclude-file-folder-or-website-from-AVG-scanning", anchor.Attributes["href"].Value);
			Assert.AreEqual("Instructions de AVG", anchor.InnerText);
			Assert.AreEqual(HtmlNodeType.Text, span.ChildNodes[2].NodeType);
			Assert.AreEqual(".", span.ChildNodes[2].InnerHtml);
		}

		[Test]
		public void TestNestedHtmlLists()
		{
			var injector = HtmlXliff.Parse(@"<html>
 <body>
  <ul>
  ...
  <body>
   ...
   <trans-unit id=""integrity.todo.ideas.Retrieve"">
    ...
    <note>ID: integrity.todo.ideas.Retrieve</note>
			...
			var li0 = ul.ChildNodes[1];
			Assert.AreEqual("li", li0.Name);
			Assert.AreEqual(3, li0.Attributes.Count);
			Assert.AreEqual("fr", li0.Attributes["lang"].Value);
			Assert.AreEqual("fr", li0.Attributes["xml:lang"].Value);
			...
			Assert.AreEqual(0, p0.Attributes.Count);
			...
			var li1 = ul.ChildNodes[3];
			Assert.AreEqual("li", li1.Name);
			Assert.AreEqual(3, li1.Attributes.Count);
			Assert.AreEqual("fr", li1.Attributes["lang"].Value);
			Assert.AreEqual("fr", li1.Attributes["xml:lang"].Value);
			...
			Assert.AreEqual(0, p1.Attributes.Count);
			...
			Assert.AreEqual(3, li1a.Attributes.Count);
			Assert.AreEqual("fr", li1a.Attributes["lang"].Value);
			Assert.AreEqual("fr", li1a.Attributes["xml:lang"].Value);
			...
			Assert.AreEqual(0, p2.Attributes.Count);
			...
			Assert.AreEqual("li", li2a.Name);
			Assert.AreEqual(3, li2a.Attributes.Count);
			Assert.AreEqual("fr", li2a.Attributes["lang"].Value);
			Assert.AreEqual("fr", li2a.Attributes["xml:lang"].Value);
			...
			Assert.AreEqual(0, p3.Attributes.Count);
			...
			Assert.AreEqual(3, li2.Attributes.Count);
			Assert.AreEqual("fr", li2.Attributes["lang"].Value);
			Assert.AreEqual("fr", li2.Attributes["xml:lang"].Value);
			...
			Assert.AreEqual(0, p4.Attributes.Count);
